...
核心机制（v12 路由语义）：
    本模块输出的是分类 routing 参数 τ_c = τ0_c + Δτ_c，再由 τ_c 生成参数化软窗 A_c(t)
    （不对称高斯，D9），软对齐读取 H_c = Σ_t A_c(t)·Z'(t)。τ 是 A 的「生成参数」而非
    「事后统计量」，分类梯度确实会到达 τ；但 v12 起 τ 只允许解释为
    `pcw_tau`（fold-calibrated routing window），生理潜伏期只接受独立的
    `measurement.LatencyMeasurement` 输出的 `measured_tau_posterior`。
    分类 BCE 能够移动 τ 不构成逐试次 latency jitter 的可识别性证据。
...
三思决策记录（供后续会话追溯）：
    D-causal-reverse 因果反转（blueprint D8，v12 降级）：τ 是 A 的生成参数，分类 BCE 会经
                     H→A→τ 监督它。该梯度只证明 τ 是分类 routing 参数；禁止把它解释为
                     生理潜伏期或 latency-jitter 可识别性。旧方案「free attention →
                     soft-argmax」作为生理测量已被推翻，作为路由读出仍需通过 PCW 诊断 gate。
    D-tau-param      τ_c = τ0_c + Δτ_c。τ0 是可学习 Parameter（数据驱动，初始先验 220/300/350ms）；
                     正式 Δτ 读出使用逐成分 attention_softargmax，再由轻量 MLP 生成有界偏移。
    ...
    D-readout-controls  global_pool/maxmean/attention 仅作为 claim-gate 的研究对照保留；它们不属于
                     正式 recipe，也不得通过默认入口启用。
...
契约（输入 → 输出）：
    Z' ∈ R^{B×T×D} → (H, tau, sigma)：
        ...
        tau   ∈ R^{B×3}    pcw_tau：分类 routing 窗口中心（ms，逐试次；非生理测量）
...
# Canonical adult PCW window priors —— 全局唯一默认源（constitution E7 / P3）。
# recipe、runner、calibrator 与测试只能 import 这里的常量，禁止重新定义数字。
PCW_CANONICAL_TAU0_MS: tuple[float, ...] = (220.0, 300.0, 350.0)
PCW_CANONICAL_TAU0_BOUNDS: tuple[tuple[float, float], ...] = (
    (180.0, 280.0),
    (250.0, 380.0),
    (280.0, 500.0),
)
PCW_CANONICAL_SIGMA_BOUNDS: tuple[tuple[float, float], ...] = (
    (20.0, 50.0),
    (20.0, 80.0),
    (20.0, 80.0),
)
PCW_CANONICAL_DTAU_BOUNDS: tuple[tuple[float, float], ...] = (
    (-30.0, 30.0),
    (-30.0, 0.0),
    (-50.0, 150.0),
)

# GTN（7–17 岁儿童，鼻参考三导）的数据集专用宽窗覆盖。它是显式 named
# override，只允许 GTN runner 引用，不得进入通用默认（recipe/factory）。
GTN_CHILD_TAU0_MS: tuple[float, ...] = (220.0, 300.0, 460.0)
GTN_CHILD_TAU0_BOUNDS: tuple[tuple[float, float], ...] = (
    (180.0, 280.0),
    (250.0, 380.0),
    (350.0, 600.0),
)
GTN_CHILD_SIGMA_BOUNDS: tuple[tuple[float, float], ...] = (
    (20.0, 50.0),
    (20.0, 80.0),
    (20.0, 150.0),
)
GTN_CHILD_DTAU_BOUNDS: tuple[tuple[float, float], ...] = PCW_CANONICAL_DTAU_BOUNDS

# Δτ 读出路径：正式 recipe 使用 attention_softargmax；其余三项仅作 claim-gate 研究对照。
#   global_pool            T 维平均（丢失潜伏期）
#   maxmean                峰值 + 平均，保留峰值幅度但无位置
#   attention              单 query 时间注意力池化，保留关键时间上下文
#   attention_softargmax   逐成分 attention + 显式时间质心 + 峰值特征（正式）
...
class ComponentWindow(nn.Module):
    """参数化成分窗：Z' (B,T,D) → H (B,3,D) + pcw_tau (B,3) + sigma (3,2)。

    ``tau`` 是分类 routing 窗口中心，不是生理潜伏期。生理潜伏期只由
    ``measurement.LatencyMeasurement`` 输出 ``measured_tau_posterior``。
    ...
    dtau_readout : str
        Δτ 的读出路径（默认 "attention_softargmax"）：见模块常量 DTAU_READOUTS。
    min_component_gap_ms : float
        强制 ``tau_N2 < tau_P3a < tau_P3b`` 的最小间隔。候选时延已满足顺序时
        保持原值，仅在越序时投影到生理可行区间。
    """
    ...
    def __init__(
        ...
        tmin: float = -200.0,
        tmax: float = 800.0,
        tau0_ms: Sequence[float] = PCW_CANONICAL_TAU0_MS,
        tau0_bounds: Sequence[tuple[float, float]] = PCW_CANONICAL_TAU0_BOUNDS,
        sigma_bounds: Sequence[tuple[float, float]] = PCW_CANONICAL_SIGMA_BOUNDS,
        dtau_bounds: Sequence[tuple[float, float]] = PCW_CANONICAL_DTAU_BOUNDS,
        smooth_w_ms: float = 10.0,
        dtau_readout: str = "attention_softargmax",
        min_component_gap_ms: float = 1.0,
    ):
        super().__init__()
        ...
        self.min_component_gap_ms = float(min_component_gap_ms)
        if self.min_component_gap_ms <= 0.0:
            raise ValueError("min_component_gap_ms must be positive.")
        ...
        absolute_tau_hi = self.tau0_hi + self.dtau_hi
        if not (
            absolute_tau_hi[0] + self.min_component_gap_ms < absolute_tau_hi[1]
            and absolute_tau_hi[1] + self.min_component_gap_ms < absolute_tau_hi[2]
        ):
            raise ValueError("tau0/dtau bounds cannot satisfy ordered N2<P3a<P3b latencies.")
    ...
    def forward(
        ...
        # τ = τ0 + Δτ, followed by an identity-when-feasible order projection.
        # This keeps the component identities stable even when custom bounds overlap.
        tau_candidate = tau0[None, :] + dtau
        tau_hi = (self.tau0_hi + self.dtau_hi).to(dtype=dtype)
        tau_n2 = tau_candidate[:, 0]
        tau_p3a = torch.minimum(
            torch.maximum(tau_candidate[:, 1], tau_n2 + self.min_component_gap_ms),
            tau_hi[1],
        )
        tau_p3b = torch.minimum(
            torch.maximum(tau_candidate[:, 2], tau_p3a + self.min_component_gap_ms),
            tau_hi[2],
        )
        tau = torch.stack((tau_n2, tau_p3a, tau_p3b), dim=1)

...
## 15. 三数据集矩阵定案：BNCI 追平 + Brain Invaders 显著超越（2026-08-24）

### 15.1 BNCI-008 根因修复（§14 后续）

嫌疑排查（单 fold 适配器协议）：base .7365 / mean_only .8177 / **zscore .8346**；
pw4 与时间抖动增强排除。**根因：σ_b 噪声放大器**——BNCI epoch 从 0ms 起、无
pre-stimulus 基线段，σ_b 只能从 4 点（16ms）估计，每试次随机缩放输入。
GTN 有 −200~0ms 真基线（51 点）故 trial 模式更优——标准化策略按数据集基线
结构选择（N2P3Net.baseline_mode + runner --input-norm，commit aaeb240）。
另发现：无早停时 30ep 末权重过拟合至 0.53（vs 早停 0.76）。

**BNCI-008 修复版全量：bacc .7102 / AUC .7993——与 EEGNet（.7103/.7992）
完全打平**，6.6pt 差距彻底消除。

### 15.2 三数据集矩阵（GLM v3.1：带通 init + BN + 门控参考 + 按数据集校准/标准化）

| 数据集（LOSO） | N2P3Net | EEGNet | 配对检验 |
|---|---|---|---|
| GTN-242（hit/AUC） | .8388/.7543 | .8395/.7620 | McNemar p=1.000 完全平局 |
| BNCI-008（bacc/AUC） | .7102/.7993 | .7103/.7992 | 完全打平 |
| **bi2014a 60被 16导** | **.6899/.7718** | .6803/.7541 | **Wilcoxon p=0.0019 显著超越**（AUC +1.8pt，40胜20负） |

bi2014a（Brain Invaders 2014a，60 被×935 试次，16 干电极缺 Fz/PO7/PO8，
校准 P3b=375ms/σ=101ms）：**首个统计显著超越 EEGNet 的数据集**。意义：
（a）16 导全量下坐标调制空间通路 + 带通时间通路的组合优势显性化；
（b）干电极场景（自采 8 导方案的目标场景）验证通过；
（c）多数据集一致打平/超越 = 架构通用性证据。

### 15.3 域迁移（GTN + aux(BNCI)，MMD λ4=0.1 + 按域参考层）

GTN-60：hit .9167 / AUC .7650 vs 纯 v3（.9167/.7691）——**不伤主任务**
（hit 持平、AUC 噪声带内）。跨域基建（N2P3NetDomainBaseline + per-domain
门控参考）验证通过；对辅助域本身的反向增益待 Phase 3 系统实验。

### 15.4 遗留

- 多模型 run 的 record.json 互相覆盖（per-model JSON 在 loso_compare 完整保留）
- PCW τ/σ 可解释性复核仍待做（前端三处改动后）
- bi2014a bacc std 0.068 偏大（被试异质性），失败被试分析待做

## 16. §15.4 三遗留项处理结果（2026-08-24）

### 16.1 多模型 run 覆盖问题（已修复）

run_bnci008_loso.py 改为每模型独立子目录 runs/<run>/<spec>/（progress.jsonl +
record.json 均不再覆盖）；launcher 监控同步支持嵌套目录（dashboard URL 用
?run=<run>/<spec>）。冒烟验证通过。

### 16.2 PCW τ/σ 可解释性复核（结论需诚实表述）

三数据集（各训一个 fold，校准先验 + 当前最佳前端）取证：

| 数据集 | P3b 真值 | τ0 init→final | 逐试次 τ target vs nontarget | σ(P3b) |
|---|---|---|---|---|
| GTN | 460ms | 460→460（drift 0） | 458±3 vs 465±6（差 −7ms） | 125ms |
| BNCI | 504ms | 504→504（drift 0） | 522±12 vs 521±14（差 +1ms） | 78ms |
| bi2014a | 375ms | 375→375（drift 0） | 372±5 vs 372±5（差 −1ms） | 121ms |

- **τ/σ 数值与 ERP 真值一致、σ 生理合理——但一致性来自校准初始化，不是网络
  学习**：τ0 三数据集 drift 全为 0（梯度饥饿依旧），逐试次 Δτ 读数无
  target/nontarget 判别力（差 ≤7ms 且方向不一致）。
- **主张修正**：可解释性表述应为「ERP 校准驱动的成分窗 + 结构化幅值读出」，
  不能声称「网络学会了成分定位」。τ/σ 是数据驱动设定的可解释参数（每次接入
  由 calibrate_erp.py 自动校准），非网络涌现的可解释性。论文写作需按此口径。

### 16.3 bi2014a 失败被试画像（bacc std 0.068 的解释）

- **逐被试 ERP SNR 与 fold AUC 相关弱**（N2P3 r=+0.156，EEGNet r=+0.181）——
  失败被试不是 ERP 信号差的被试，是被试级域偏移（形态/潜伏期个体差异超出
  跨被试训练覆盖）。
- 最差被试：subj52 双双随机（N2P3 0.508 / EEGNet 0.512，SNR 0.46 却中位数
  以上——疑似数据质量问题被试）；subj2 是我方最大单败（0.601 vs 0.731）。
- **胜负结构**：胜 40 被试（均差 +3.8pt）/ 负 20（均差 −2.3pt）——优势来自
  多数被试的小稳定增益 + 个别大胜，不是单一结构。
- **重要发现**：低 SNR 四分位被试上我方优势更大（0.757 vs 0.729，+2.8pt）
  高于高 SNR 四分位（+0.9pt）——**生理先验在难被试上价值更高**（可写入论文）。

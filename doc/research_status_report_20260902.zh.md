...
decisions 为校准（zero\_shot 未使用），后续 30 个未知 decisions 按
evidence level 1..8 记分，estimand
`known_early_decisions_to_later_unknown_decisions`，split\_axis
...
- 修复（`ab6c8e7`，已过全量测试 + Ruff）：
  ...
  - promotion matrix DAG 由 96 个单 checkpoint 任务改为 12 个
    checkpoint\_batch 任务；resume 时重验 checkpoint 字节与合同 digest；
...
- 未闭合：96 个 checkpoint（每个约 24 KB）、
  ...
  digest 与新代码不兼容（checkpoint->checkpoint\_batch），

...
PARENT_RUN_ROOT=${PARENT_RUN_ROOT:-$ROOT/artifacts/multidomain_joint_20260901}
RUN_ROOT=${RUN_ROOT:-$ROOT/artifacts/multidomain_weighted_20260901}
MAX_JOBS=${MAX_JOBS:-8}
cd "$ROOT"
...
MANIFEST=experiments/multidomain_weighted_amendment_20260901.json
ARM=bi_bnci_joint_bi3_bnci1_car5
...
parent_result_count=$(find "$PARENT_RUN_ROOT/results" -maxdepth 1 -type f -name '*.json' | wc -l)
if (( parent_result_count != 24 )); then
  echo "PARENT_RESULT_COUNT expected=24 actual=$parent_result_count" >&2
  exit 3
fi
cp "$PARENT_RUN_ROOT/results"/*.json "$RUN_ROOT/results/"
sha256sum "$TARGET_CACHE" "$JOINT_CACHE" "$MANIFEST" "$PARENT_RUN_ROOT/analysis.json" \
  "$PARENT_RUN_ROOT/results"/*.json "$BLOCK_DIR"/*.json >"$RUN_ROOT/input_sha256.txt"
...
for seed in 20260901 20260902 20260903; do
  for block in 0 1 2 3; do
    ...
    launch "$name" env CUDA_VISIBLE_DEVICES=0 "$PYTHON" experiments/run_pretrain_supervised.py \
      ...
      --batch-size 512 \
      --seed "$seed" \
      --subject-prefix-repeat 'BI::=3,BNCI::=1' \
      --qc-ptp-uv 0 \

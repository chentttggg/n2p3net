...
def build_boundary_candidates(*, base_batch_size: int) -> list[SensitivityCandidate]:
    candidates = [
        SensitivityCandidate(
            name="baseline",
            axis="baseline",
...
def build_kernel_range_candidates(*, base_batch_size: int) -> list[SensitivityCandidate]:
    base_kernel = DEFAULT_N2P3_ARCHITECTURE.temporal_kernel_size
    return [
        SensitivityCandidate(
            name=f"temporal_kernel_size_{kernel}",
...
def build_secondary_candidates(*, base_batch_size: int) -> list[SensitivityCandidate]:
    candidates = build_candidates(base_batch_size=base_batch_size)
    output: list[SensitivityCandidate] = []
    for candidate in candidates:
        if candidate.axis == "temporal_kernel_size":
            continue
        output.append(
            replace(
                candidate,
                architecture_overrides={
                    ...
                    **candidate.architecture_overrides,
                },
            )
        )
    return output
